...
## 9. 학습 곡선 실험 추가 (2026-07-04, 실행 대기)

- train 1k/2k/4k/전체 슬라이스별 LSTM·TF-IDF 학습 → test acc 곡선 + DistilBERT 참고선 (`03_learning_curve.{png,csv}`).
- 가설: 비사전학습 모델은 데이터 증가의 수혜가 크고(임베딩을 새로 배움), 사전학습 모델은 소량으로 포화 — "사전학습 = 데이터를 공짜로 얻는 효과".
...
| 모델 | Accuracy | F1 | 데이터 | 비고 |
|---|---|---|---|---|
| TF-IDF+LogReg | 0.818 | 0.813 | 균형 1만 | 학습 수 초 |
| LSTM | 0.763 | 0.753 | 균형 1만 | early stopping, best epoch 21 |
| DistilBERT | 0.903 | 0.941 | 불균형 2만* | *균형 1만 재학습 예정 |

## 남은 작업

- [ ] 04 DistilBERT 균형 1만 건 재학습 → 05 비교표·보고서 수치 갱신
- [ ] 03 학습 곡선 셀 실행
- [ ] 06 RAG 인덱스 재구축, 07 모델 해석 실행 → 보고서 placeholder 교체
