...
## 10. 게임별 분석 기능 추가 (2026-07-04)

- "게임 이름 → 그 게임의 긍/부정 분류 + 장단점 요약" 흐름 구현.
- `clean_reviews(extra_cols=["app_name"])`로 02에서 게임명 유지 → 06 색인 시 `app_name`/`label` 메타데이터 저장 → 앱이 Chroma 메타데이터만으로 게임 목록·게임별 리뷰를 얻음 (배포 환경에서 `data/` 불필요).
- 앱 3탭 개편: ① 게임 분석(리뷰 20건 이상 게임 선택 → 분류+요약), ② 리뷰 직접 입력(기존), ③ RAG Q&A + 게임 필터.
- `src/rag/qa.py` `answer(question, app_name=None)` — Chroma `where` 필터로 특정 게임 리뷰만 검색.
- 현재 샘플 기준 리뷰 20건 이상 게임 92개 (PAYDAY 2 174건, DayZ 161건, Rust 139건 등).
- **혼합 샘플링 확장 (2026-07-04)**: 학습 데이터는 그대로 두고 색인만 확장 — train 5,686 + 인기 상위 20개 게임(원본 641만에서 라벨당 200건씩 정제 후 5,146건) = **10,810건 색인**. 총 89MB(최대 파일 48MB)로 git 100MB 제한 안전, 용량 assert 가드 추가. Portal 2·Witcher 3·Undertale 등 유명 게임의 게임당 리뷰가 수백 건으로 확대. 전체(641만) 색인은 임베딩 ~5시간 + 수십 GB + Streamlit Cloud 메모리 초과로 기각.

## 11. LLM 전환 (2026-07-03)

- Qwen2.5-7B-Instruct → Qwen3-30B-A3B-Instruct-2507로 전환했으나, 배포 검증 과정(2026-07-04)에서 HF Inference 라우터가 이 모델을 계정에 제공하지 않음을 확인 (`model_not_supported`). 라우터 제공 목록으로 실호출 테스트 후 gemma-3-27b-it를 임시 사용했으나, **Qwen/Qwen2.5-7B-Instruct**(1차) + **meta-llama/Llama-3.3-70B-Instruct**(폴백)로 최종 확정 (2026-07-05, 총평·장단점 한국어 출력 검증). Qwen3.5/3.6 계열은 응답이 빈 문자열(reasoning 토큰 소모)이라 제외.
...
## 12. LLM·RAG를 LangChain으로 전환 (2026-07-05)

- 직접 구현(InferenceClient + chromadb 네이티브 + f-string)을 **LangChain 표준 구조**로 재작성 — 구조 이해도 향상 목적.
- `client.py`: `ChatHuggingFace` + `with_fallbacks`(gemma→Qwen2.5 폴백), `ChatOllama` 로컬 전환 유지.
- `index.py`: `HuggingFaceEmbeddings` + `langchain_chroma.Chroma` (기존 chroma_store 그대로 읽음, 재색인 불필요).
- `qa.py`/`summarize.py`: LCEL 체인 `prompt | llm | StrOutputParser`, retriever의 `filter`로 게임 필터.
- 공개 API(chat, answer, summarize, get_collection) 시그니처 유지 → app.py 무변경. QA·요약·게임 집계 전 경로 실호출 검증 통과.
- requirements: langchain-core/huggingface/chroma/ollama 추가, 미사용 faiss-cpu 제거.

## 13. LLM 폴백을 Qwen2.5 계열로 통일 (2026-07-07)

- 폴백을 `meta-llama/Llama-3.3-70B-Instruct` → `Qwen/Qwen2.5-72B-Instruct`로 교체. 1차(`Qwen2.5-7B-Instruct`)와 폴백 모두 Qwen2.5 계열로 통일해 응답 톤/포맷 일관성을 확보하고 모델군 혼재를 정리.
- `src/config.py`(`LLM_FALLBACK_ID`) 외 `README.md`, `app.py`, `docs/rag_guide.md` 문서 표기 동기화.
...
- [x] 게임별 분석 반영 재실행 완료 (2026-07-04): 02(app_name 컬럼) → 06(메타데이터 색인 5,686건, PAYDAY 2 필터 검색 검증). 로그오즈비 문서 빈도 차트(v4)도 이때 생성 → 보고서 반영

- [x] 04 DistilBERT 균형 1만 건 재학습 (acc 0.856) → 05 비교표·보고서 반영 완료
- [x] 03 학습 곡선 실행 → 보고서 반영 완료
- [x] 06 RAG 인덱스 재구축 — 이후 2,000건 샘플 → **train 전체(5,686건) 색인**으로 확대 (2026-07-04, 재구축 시 컬렉션 재생성 + 배치 add로 `build_index` 보강)
