...
- Qwen2.5-7B-Instruct → Qwen3-30B-A3B-Instruct-2507로 전환했으나, 배포 검증 과정(2026-07-04)에서 HF Inference 라우터가 이 모델을 계정에 제공하지 않음을 확인 (`model_not_supported`). 라우터 제공 목록으로 실호출 테스트 후 **google/gemma-3-27b-it**(1차, 한국어 출력 검증) + **Qwen/Qwen2.5-7B-Instruct**(폴백)로 최종 확정. Qwen3.5/3.6 계열은 응답이 빈 문자열(reasoning 토큰 소모)이라 제외.
